...
export async function GET(request: NextRequest) {
  ...
  try {
    const hashtags = await provider.getTrendingHashtags({
      region,
      industry,
      period,
    });
    return NextResponse.json({ hashtags, source: provider instanceof TikTokCreativeProvider ? "apify" : "scrapecreators" });
  } catch (e: any) {
    console.error("[TIKTOK HASHTAGS] Error:", e);
