      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-[var(--color-border)]">
          <div className="flex-1 min-w-0">
            <h2 className="text-sm font-semibold truncate" style={{ fontFamily: "var(--font-display)", color: "#FFFFFF" }}>
              {item.title}
            </h2>
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-[9px] font-medium" style={{ color: "#FFFFFF60" }}>{item.creator_name}</span>
              <span className="text-[9px]" style={{ color: "#FFFFFF30" }}>·</span>
              <span className="text-[9px]" style={{ color: "#FFFFFF60" }}>
          ...
          <div className="flex items-center gap-1">
            <button onClick={onToggleFavorite} className="p-1.5 hover:bg-[var(--color-base)] transition-colors">
              <Heart size={13} className={item.is_favorite ? "fill-[#EF4444] text-[#EF4444]" : ""} style={{ color: "#FFFFFF60" }} />
            </button>
        ...
        <div className="flex-1 overflow-y-auto grid grid-cols-[1fr_280px] divide-x divide-[var(--color-border)]">
          ...
          <div className="flex items-center justify-center p-4 min-h-[300px]" style={{ backgroundColor: "var(--color-base)" }}>
            {item.type === "image" ? (
              ...
              <div className="flex flex-col items-center gap-3 text-center">
                ...
                <p className="text-xs" style={{ color: "#FFFFFF60" }}>Aperçu vidéo</p>
                {item.duration && (
                  <p className="text-[10px]" style={{ color: "#FFFFFF40" }}>
              ...
              <div className="flex flex-col items-center gap-3 text-center">
                ...
                <p className="text-xs" style={{ color: "#FFFFFF60" }}>Fichier audio</p>
                {item.duration && (
                  <p className="text-[10px]" style={{ color: "#FFFFFF40" }}>
                    Durée : {Math.floor(item.duration / 60)}min {item.duration % 60}s
                  </p>
              ...
              <div className="flex flex-col items-center gap-3 text-center">
                <FileTextIcon />
                <p className="text-xs" style={{ color: "#FFFFFF60" }}>Document</p>
                <a href={item.url} target="_blank" rel="noopener noreferrer"
                  className="flex items-center gap-1 text-[10px] text-[#C75B39] underline underline-offset-4">
          ...
          <div className="p-4 space-y-4 overflow-y-auto">
            ...
            {/* AI Analysis */}
            {item.ai_analyzed && (
              <Section title="Analyse IA" icon={<Sparkles size={11} style={{ color: "#10B981" }} />}>
                {/* Description */}
                {item.ai_description && (
                  <p className="text-[10px] leading-relaxed" style={{ color: "#FFFFFF80" }}>
                    {item.ai_description}
                  </p>
                )}
                ...
                {moodLabel && (
                  <div className="flex items-center gap-1.5 mt-2">
                    ...
                    <span className="text-[9px] font-medium" style={{ color: "#FFFFFF80" }}>{moodLabel.label}</span>
                  </div>
                )}
                ...
                {item.ai_suitable_platforms.length > 0 && (
                  <div className="mt-2">
                    <p className="text-[8px] uppercase tracking-wider font-medium mb-1" style={{ color: "#FFFFFF50" }}>
                      Plateformes adaptées
                    </p>
                    <div className="flex flex-wrap gap-1">
                      {item.ai_suitable_platforms.map((p) => (
                        <span key={p} className="px-1.5 py-0.5 text-[8px] font-medium"
                          style={{ backgroundColor: `${PLATFORM_COLORS[p] ?? "#666"}20`, color: PLATFORM_COLORS[p] ?? "#FFF" }}>
                          {PLATFORM_LABELS[p] ?? p}
                        </span>
                      ))}
                    </div>
            ...
            {item.moderation_checked && (
              <Section title="Modération" icon={<Shield size={11} style={{ color: item.moderation_safe ? "#10B981" : "#EF4444" }} />}>
                {item.moderation_safe ? (
                  <div className="flex items-center gap-1.5">
                    <CheckCircle size={10} style={{ color: "#10B981" }} />
                    <span className="text-[10px]" style={{ color: "#10B981" }}>Contenu sûr — OK pour publication</span>
                  </div>
                ) : (
                  <div className="space-y-1.5">
                    <div className="flex items-center gap-1.5">
                      <AlertTriangle size={10} style={{ color: "#EF4444" }} />
                      <span className="text-[10px] font-medium" style={{ color: "#EF4444" }}>Attention nécessaire</span>
                    </div>
                    {item.moderation_concerns.map((c, i) => (
                      <p key={i} className="text-[9px] pl-4" style={{ color: "#EF4444CC" }}>• {c}</p>
                    ))}
                    <p className="text-[8px] mt-1" style={{ color: "#FFFFFF40" }}>
            ...
            <div className="pt-2 space-y-2">
              ...
              <button
                ...
                className="flex items-center justify-center gap-1.5 w-full py-2 text-[9px] font-medium uppercase tracking-wider border border-[#EF4444]/30 text-[#EF4444] hover:bg-[#EF4444]/10 transition-all"
              >
                <Trash2 size={10} />
                Supprimer
              </button>
  ...
  return (
    <div>
      <div className="flex items-center gap-1.5 mb-1.5">
        {icon}
        <h3 className="text-[9px] uppercase tracking-wider font-semibold" style={{ color: "#FFFFFF80" }}>{title}</h3>
      </div>
      {children}
    </div>
  );
...
function InfoRow({ icon, label, value }: { icon: React.ReactNode; label: string; value: string }) {
  return (
    <div className="flex items-center justify-between py-0.5">
      <span className="flex items-center gap-1.5 text-[9px]" style={{ color: "#FFFFFF60" }}>
        {icon}
        {label}
      </span>
      <span className="text-[9px] font-mono" style={{ color: "#FFFFFF80" }}>{value}</span>
    </div>
  );
}
...
function FileTextIcon() {
  return (
    <svg width="32" height="32" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" style={{ color: "#FFFFFF40" }}>
