  ...
  return (
    <div className="space-y-6 animate-fade-in">
      <div className="flex items-center gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold" style={{ fontFamily: "var(--font-display)", color: "#FFFFFF" }}>Segments</h1>
          <p className="text-sm mt-1" style={{ color: "#FFFFFF" }}>Segmente ta base fans pour des campagnes ciblées</p>
        </div>
      </div>

      {/* Preset segments */}
      <div>
        <h3 className="text-sm font-semibold mb-3" style={{ color: "#FFFFFF" }}>Segments prédéfinis</h3>
          ...
          {SEGMENT_PRESETS.map((seg) => (
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-sm font-semibold" style={{ color: "#FFFFFF" }}>{seg.name}</h4>
                  <p className="text-xs mt-0.5" style={{ color: "#FFFFFF" }}>{seg.description}</p>
                </div>
                <ChevronRight size={16} style={{ color: "#C75B39" }} />
              </div>
              <div className="flex gap-1 mt-2">
                {seg.filters.map((f, i) => (
                  <span key={i} className="text-[10px] px-1.5 py-0.5 rounded-sm" style={{ background: "rgba(199,91,57,0.1)", color: "#C75B39" }}>
      ...
      <div className="p-4 border border-[var(--color-border)]" style={{ backgroundColor: "var(--color-card)" }}>
        <h3 className="text-sm font-semibold mb-3" style={{ color: "#FFFFFF" }}>Créer un segment personnalisé</h3>
